/* ECU_TESTER :: live.js — wires the device's binary WebSocket stream into the
   dashboard. protocol.js decodes the frames (docs/PROTOCOL.md); this file maps
   the decoded fields onto the ECU display API exposed by app.js.

   Analog fields arrive as raw mV at the ECU pin (PROTOCOL.md §3.1); the mini
   gauges display volts, so the only scaling here is ÷1000. CTS and IGF have no
   telemetry field in protocol v1 — their needles stay at rest until the
   protocol grows those channels. Same for the CURRENT readout (no field yet)
...
// coil/injector pulse is lost, and apply on a 80 ms tick (~12.5 Hz).
let pending = null;

// live counters read by the diagnostic overlay (js/diag.js)
const stats = (window.__ecuStats = { msgs: 0, telem: 0, lastTelemMs: 0, msgRate: 0 });
...
// against real hardware, override with ?device=10.10.10.10 (or open file://).
const params = new URLSearchParams(location.search);
const host = params.get("device") || location.host || "10.10.10.10";

const socket = new EcuSocket(`ws://${host}/ws`, {
  onFrame,
  onStatus: (st) => {
...
setInterval(() => {
  if (
    socket.ws && socket.ws.readyState === WebSocket.OPEN &&
